"""
=============================================================
DevPulse -- Phase 1: Bare-bones MCP client test
Proves initialize() -> list_tools() -> call_tool() end to end,
...
async def main():
    ...
    # process's environment variables. Without passing env=os.environ.copy(),
    # the server subprocess won't see GITHUB_TOKEN (or, in some sandboxes, even
    # basic things like SSL_CERT_FILE) -- a common gotcha worth internalizing.
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            print(
                "    Resources: "
                f"{[r.uri for r in resources_result.resources] if resources_result.resources else '(template-based, not listed directly)'}"
            )
